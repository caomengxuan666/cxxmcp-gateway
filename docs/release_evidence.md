...
| Future capability extension gate | `docs/capability_extension_gate.md`, `docs/capability_design_template.md` |
...
| Runtime component is explicit | `tests/fixtures/package_smoke/runtime_consumer.cpp` runtime-config-to-options mapping, move construction/assignment, typed runtime APIs, hosted endpoint option validation plus successful start/stop, and observer coverage; missing-runtime component failure tests |
...
| CLI/config runtime knobs | `gateway_cli_help`, `gateway_cli_invalid_args`, `gateway_cli_config_prewarm_from_file`, `gateway_cli_runtime_config_overrides`, CLI merged runtime-config validation, `gateway_config_io`, `tests/fixtures/package_smoke/config_io_consumer.cpp` |
...
| Graceful stop and concurrent wait/stop | runtime stop, active-call stop, direct API and raw routed request stopped/stopping checks for process-stdio and Streamable HTTP paths, wait/stop overlap tests, `test_runtime_stop_timeout_bounds_active_stdio_call_wait`, `test_persistent_pool_stop_waits_for_timed_out_stdio_call` |
